...
public class Epidemic {
    ...
    static double doRep() {
        ...
        for (int day = 1; day <= Params.numDays; day++) {
        	...
        	quarantined = Employee.stateCounts.get("quarantined");
        	cost = cost + (infected + quarantined) * Params.C;
        	if (debug == 2) {
        	...
        	for (int j = 0; j < employees.length; j++) {
        		employees[j].transition(infected, unigen);
        	}
        }
        if (debug == 2) {
        	System.out.println();
...
class Employee {
	...
	public void transition(int infected, Clcg4 unigen) {
		double randnum = unigen.nextValue(Params.generator);
//		System.out.println(randnum);
		double p = 1 - (1 - alpha0) * (Math.pow((1 - alpha), infected));
